...
OLD
  ( unset FIRECRAWL_API_KEY HERMES_UI_URL VERTICAL INSTANCE_TITLE \
          CORTEX_API_KEY HERMES_UI_HOSTNAME DASHBOARD_BIND HIA_BASE_URL NEWSLETTER_BASE_URL POPBYS_BASE_URL
...
test_cortex_api_key_forwarded() {
  ...
  ( unset CORTEX_API_KEY FIRECRAWL_API_KEY HERMES_UI_URL VERTICAL INSTANCE_TITLE \
          HERMES_UI_HOSTNAME DASHBOARD_BIND HIA_BASE_URL NEWSLETTER_BASE_URL POPBYS_BASE_URL
    render_stack_env "$new" "$old" )
...
POPBYS_BASE_URL=https://popbys.example.com
REPORTS_DIR=/reports
REPORTS_HOST_DIR=/home/ollie/.hermes/profiles/olivia-marketing/workspace
OLD
  ( unset HERMES_UI_HOSTNAME DASHBOARD_BIND HIA_BASE_URL NEWSLETTER_BASE_URL POPBYS_BASE_URL \
  ...
  assert_eq "POPBYS_BASE_URL preserved"    "$(grep -E '^POPBYS_BASE_URL=' "$new" | cut -d= -f2-)" "https://popbys.example.com"
...
OLD
  ...
  for k in OAUTH2_PROXY_CLIENT_ID OAUTH2_PROXY_CLIENT_SECRET OAUTH2_PROXY_COOKIE_SECRET \
           ...
           DASHBOARD_BIND HIA_BASE_URL NEWSLETTER_BASE_URL POPBYS_BASE_URL REPORTS_DIR REPORTS_HOST_DIR \
  ...
  assert_eq "popbys base url preserved" "$(grep -E '^POPBYS_BASE_URL=' "$new" | cut -d= -f2-)" "https://popbys"
  rm -f "$old" "$new"
}
